        ...
        for _ in range(len(self.api_keys)): # Try all primary keys once
            try:
                ...
                if self._is_auth_error(e) and self.rotate_key():
                    logger.warning(f"Primary API key failed authentication, trying next backup key. Details: {e}")
                    continue
        ...
        if not self.client:
            ...
            try:
                success, gwen_response = call_gwen_ai(
                    "Act as a professional media analyst. Analyze the following news and provide "
