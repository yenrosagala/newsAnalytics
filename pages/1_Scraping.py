...
from app.services.decision_brief import parse_decision_brief_json, serialize_brief, deserialize_brief
...
st.markdown("# 📥 News Scraper & Management")
st.caption("Google News Scraping, Executive Summary, & Story Clustering")
st.markdown("---")
...
def process_and_get_pdf(target_keyword):
    ...
    if cached_summary:
        brief = deserialize_brief(cached_summary, fallback_title=target_keyword)
    else:
        formatted_articles = [
        ...
        try:
            raw_ai_response = ai_service.generate(prompt)
            brief = parse_decision_brief_json(raw_ai_response, fallback_title=target_keyword)

            db_service.save_executive_summary_to_db(
                kata_kunci=target_keyword,
                rentang_waktu=date_range_str,
                hasil_summary=serialize_brief(brief)
            )
...
# --- MEMBUAT TABS UTAMA ---
tab_scrape, tab_database, tab_cluster = st.tabs([
    "🚀 News Scraper", 
    "🗄️ Database & Summary", 
...
with tab_scrape:
    ...
    # Bagian otomatis download setelah scraping di dalam Tab 1
    ...
    if active_keyword_to_summarize:
        ...
        with st.spinner("Menyiapkan dokumen PDF laporan resmi..."):
            ...
            if pdf_data:
                ...
                st.error(err_msg)
# ==========================================
# TAB 3: STORY CLUSTERING & TIMELINE
# ==========================================
with tab_cluster:
    ...
    if df_for_cluster.empty:
        ...
        with col_ck2:
            similarity_threshold = st.slider(
                ...
                help="Makin kecil = pengelompokan makin ketat. Makin besar = pengelompokan makin longgar.",
...
# ==========================================
# TAB 2: DATABASE & EXECUTIVE SUMMARY
# ==========================================
with tab_database:
