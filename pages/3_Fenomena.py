    ...
    if run_analysis:
        ...
        try:
            ...
            if not result_tree or not isinstance(result_tree, list):
                ...
                # NOTE: tidak lagi di-gate dengan `if ai_service.client`, supaya
                # fallback ke Gwen AI di dalam ai_service.generate() benar-benar
                # jalan saat Gemini client belum terkonfigurasi / semua key gagal.
                try:
                    ...
                    logger.error(f"Gagal menyusun ringkasan eksekutif (primer & fallback Gwen): {llm_err}")
                    st.warning(f"⚠️ Gagal menyusun ringkasan eksekutif via AI/Gwen ({llm_err}). Analisis per-level tetap tersedia di bawah.")
